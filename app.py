...
# -----------------------------
# CONFIGURAZIONE (Deve essere la primissima riga Streamlit)
# -----------------------------
st.set_page_config(
...
def safe_bool(v, default=True) -> bool:
    ...
    if isinstance(v, str):
        s = v.strip().lower()
        if s in ("true", "t", "1", "yes", "y", "si", "sì"): return True
        if s in ("false", "f", "0", "no", "n"): return False
    return default

def format_time_estimate(seconds: float) -> str:
    if seconds is None or seconds <= 0: return "n.d."
...
def heuristic_estimate_seconds(n_members: int, n_days: int, horizon_hours: int, total_work_h: float) -> float:
    if n_members <= 0 or n_days <= 0: return 0.0
...
# -----------------------------
def power_curve_mw(wind_ms: float, rated_mw: float = 2.0, cut_in: float = 3.0, rated: float = 12.0, cut_out: float = 25.0) -> float:
    if wind_ms is None or (isinstance(wind_ms, float) and np.isnan(wind_ms)): return 0.0
    w = float(wind_ms)
    if w < cut_in: return 0.0
    if w < rated:
        x = (w - cut_in) / (rated - cut_in)
        return rated_mw * (x ** 3)
    if w < cut_out: return rated_mw
    return 0.0
...
def plot_power_curve() -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=wind, y=power, mode="lines", line=dict(color="rgba(34,197,94,1)", width=3), name="Power Curve", hovertemplate="Vento: %{x:,.1f} m/s<br>Potenza: %{y:,.2f} MW<extra></extra>"))
    fig.update_layout(title="Power Curve (2 MW)", xaxis_title="Wind speed [m/s]", yaxis_title="Power [MW]", template="plotly_white", height=320, margin=dict(l=10, r=10, t=60, b=10), hovermode="x unified", separators=",.")
    return fig
...
def generate_price_series(ts: pd.Series, seed: int = 7) -> np.ndarray:
    ...
    base = 70 + 10 * np.sin((hour - 6) / 24 * 2 * np.pi) + 25 * np.exp(-((hour - 20) ** 2) / (2 * 2.8**2)) + 10 * np.exp(-((hour - 9) ** 2) / (2 * 2.8**2))
    weekend_factor = np.where(dow >= 5, -7, 0)
    noise = rng.normal(0, 3.0, size=len(ts))
    return np.round(np.clip(base + weekend_factor + noise, 60, 120), 2)
...
def fetch_open_meteo_ensemble(latitude: float, longitude: float, model: str, forecast_days: int, include_gusts: bool, timezone: str = "auto") -> pd.DataFrame:
    hourly_vars = ["wind_speed_80m"]
    if include_gusts: hourly_vars.append("wind_gusts_10m")
    
    # Formattazione latitudine e longitudine a 2 decimali
    params = {
    ...
    if "hourly" not in js or "time" not in js["hourly"]: raise ValueError("Risposta Open-Meteo non contiene 'hourly.time'.")
    hourly = js["hourly"]
    ...
    if not wind_keys: raise ValueError(f"Impossibile mappare membri ensemble wind_speed_* per '{model}'.")
...
def generate_mock_open_meteo_ensemble(start=None, days=10, n_members=30, seed=42, include_gusts=True) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    if start is None: start = pd.Timestamp.now().normalize().strftime("%Y-%m-%d %H:%M")
    ...
    w_base = 7.5 + 1.8 * np.sin(hour / 24 * 2 * np.pi) + 1.2 * np.sin(day_index / 7 * 2 * np.pi) + 0.6 * np.sin((hour - 14) / 24 * 4 * np.pi)
    sigma = np.clip(np.where(di < 3, 0.9, 0.9 + (di - 2) * (3.2 / max(1, (days - 3)))), 0.9, 4.0)

    for m in range(n_members):
        member_bias = rng.normal(0, 0.4)
        wind = np.clip(w_base + member_bias + rng.normal(0, sigma, size=len(df)), 0, 30)
        df[f"wind_speed_80m_member_{m}"] = np.round(wind, 2)
        if include_gusts:
            gust = np.clip(wind + np.abs(rng.normal(3.5, 1.4, size=len(df))) + rng.normal(0, sigma * 0.25, size=len(df)), 0, 40)
            df[f"wind_gusts_10m_member_{m}"] = np.round(gust, 2)
    return df

def detect_members(df: pd.DataFrame) -> Tuple[List[str], Optional[List[str]]]:
    wind_cols = sorted([c for c in df.columns if c.startswith("wind_speed_80m_member_")], key=lambda c: int(c.split("_")[-1]))
    gust_cols = sorted([c for c in df.columns if c.startswith("wind_gusts_10m_member_")], key=lambda c: int(c.split("_")[-1]))
    if not gust_cols or len(gust_cols) != len(wind_cols): gust_cols = None
    return wind_cols, gust_cols

# -----------------------------
# PLOTS
# -----------------------------
def plot_wind_speed_ensemble(df_view: pd.DataFrame, wind_cols: List[str], horizon_start: pd.Timestamp, forecast_end: pd.Timestamp) -> go.Figure:
    ...
    fig = go.Figure()
    # P90 invisibile usata come tetto
    fig.add_trace(go.Scatter(x=df_view["timestamp"], y=p90, mode="lines", line=dict(width=0), showlegend=False, hoverinfo="skip"))
    # P10 con fill fino alla P90. Il testo nel tooltip usa y=P10 e text=P90
    fig.add_trace(go.Scatter(
        x=df_view["timestamp"], y=p10, mode="lines", line=dict(width=0),
        fill="tonexty", fillcolor="rgba(14,165,233,0.18)", name="Incertezza (P10–P90)",
        hovertemplate="Tempo: %{x}<br>P10: %{y:,.2f} m/s<br>P90: %{text:,.2f} m/s<extra></extra>",
        text=p90
    ))
    fig.add_trace(go.Scatter(
        x=df_view["timestamp"], y=mean, mode="lines",
        line=dict(color="rgba(14,165,233,1)", width=2.5), name="Media Ensemble",
        hovertemplate="Tempo: %{x}<br>Media: %{y:,.2f} m/s<extra></extra>"
    ))
    
    # Finestra Monte Carlo
    fig.add_vrect(x0=horizon_start, x1=forecast_end, fillcolor="rgba(255, 165, 0, 0.08)", line_width=0, annotation_text="Area Simulata", annotation_position="top left", layer="below")
    
    fig.update_layout(title="Velocità vento prevista [m/s] (ensemble)", xaxis_title="Tempo", yaxis_title="m/s", template="plotly_white", height=320, hovermode="x unified", margin=dict(l=10, r=10, t=60, b=10), separators=",.")
    return fig

def plot_expected_production(df_view: pd.DataFrame, wind_cols: List[str], horizon_start: pd.Timestamp, forecast_end: pd.Timestamp) -> go.Figure:
    wind_mat = df_view[wind_cols].to_numpy(dtype=float)
    power_mat = np.vectorize(power_curve_mw)(wind_mat)
    ...
    fig.add_trace(go.Scatter(
        x=df_view["timestamp"], y=p10, mode="lines", line=dict(width=0),
        fill="tonexty", fillcolor="rgba(59,130,246,0.18)", name="Incertezza (P10–P90)",
        hovertemplate="Tempo: %{x}<br>P10: %{y:,.2f} MW<br>P90: %{text:,.2f} MW<extra></extra>",
        text=p90
    ))
    fig.add_trace(go.Scatter(
        x=df_view["timestamp"], y=mean, mode="lines",
        line=dict(color="rgba(59,130,246,1)", width=2.5), name="Media Ensemble",
        hovertemplate="Tempo: %{x}<br>Media: %{y:,.2f} MW<extra></extra>"
    ))
    
    # Finestra Monte Carlo
    fig.add_vrect(x0=horizon_start, x1=forecast_end, fillcolor="rgba(255, 165, 0, 0.08)", line_width=0, layer="below")
    
    fig.update_layout(title="Produzione prevista [MW] (ensemble + power curve)", xaxis_title="Tempo", yaxis_title="MW", template="plotly_white", height=320, hovermode="x unified", margin=dict(l=10, r=10, t=60, b=10), separators=",.")
    return fig

def plot_cost_candles(summary_for_plot: pd.DataFrame) -> go.Figure:
    if summary_for_plot.empty: return go.Figure()
    ...
    fig.add_trace(go.Bar(
        x=x, y=p90-p10, base=p10,
        marker=dict(color="rgba(99,102,241,0.35)"), name="Intervallo P10–P90",
        hovertemplate="D0: %{x}<br>P10: %{base:,.2f} €<br>P90: %{y+base:,.2f} €<extra></extra>"
    ))
    fig.add_trace(go.Scatter(
        x=x, y=mean, mode="lines+markers", line=dict(color="rgba(99,102,241,1)", width=2),
        marker=dict(size=6, color="rgba(99,102,241,1)"), name="Costo medio",
        hovertemplate="D0: %{x}<br>Costo medio: %{y:,.2f} €<extra></extra>"
    ))
    fig.update_layout(title="Costo totale vs D0 (candela P10–P90)", xaxis_title="Giorno di inizio D0", yaxis_title="Costo Totale (€)", template="plotly_white", height=420, margin=dict(l=10, r=10, t=60, b=10), hovermode="x unified", barmode="overlay", separators=",.")
    return fig
...
def plot_gantt_fraction(frac: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    for name, color in [("Lavoro", "rgba(34,197,94,0.85)"), ("Standby", "rgba(245,158,11,0.85)"), ("Attesa (no gru)", "rgba(59,130,246,0.30)"), ("Stop Notte", "rgba(148,163,184,0.85)")]:
        if name in frac.columns:
            fig.add_trace(go.Bar(x=frac["timestamp"], y=frac[name], name=name, marker_color=color))
    fig.update_layout(barmode="stack", title="Gantt medio (quota scenari per stato orario)", template="plotly_white", height=380, hovermode="x unified", margin=dict(l=10, r=10, t=60, b=10), separators=",.")
    return fig
...
def simulate_single_start_day_cost(df: pd.DataFrame, start_day: pd.Timestamp, wind_cols: List[str], gust_cols: Optional[List[str]], steps: List[Step], params: CraneParams, rated_mw: float = 2.0) -> Dict:
    ...
    for m, wind_col in enumerate(wind_cols):
        ...
        crane_cost, lost_revenue = 0.0, 0.0
        idx = d0_idx
        ...
        while idx < len(df) and step_i < len(steps):
            ...
            if not in_work_shift(ts, params.shift_start, params.shift_end):
                ...
                window_ok = True if current_step_started else check_min_window(df, idx, wind_col, gust_col, step, params)
            ...
            logs.append({"timestamp": ts, "state": state, "crane_cost_eur": c_cost, "prod_loss_eur": loss_eur, "step_name": step.name, "member": m, "crane_present": crane_present})
            idx += 1

        total_cost = mob_demob_apply + crane_cost + lost_revenue
        member_rows.append({"member": m, "total_cost_eur": total_cost, "mob_demob_eur": mob_demob_apply, "crane_cost_eur": crane_cost, "lost_revenue_eur": lost_revenue, "partial": (step_i < len(steps)), "completion_ts": last_ts})
        member_logs.append(pd.DataFrame(logs))

    return {"status": "ok", "start_day": d0_ts, "member_results": pd.DataFrame(member_rows), "member_logs": member_logs}
...
def compute_daily_summary_cost(all_sims: Dict[pd.Timestamp, Dict], structural_infeasible: Dict[pd.Timestamp, bool]) -> pd.DataFrame:
    ...
    for d0, sim in all_sims.items():
        ...
        p10, p90 = safe_percentile(costs, 10), safe_percentile(costs, 90)
...
def choose_optimal_day_cost(summary: pd.DataFrame, last_possible_start: Optional[pd.Timestamp], risk_aversion: float = 0.7) -> Tuple[Optional[pd.Timestamp], pd.DataFrame, str]:
    s = summary.copy()
    if s.empty: return None, s, "Nessun dato"
    mean = s["Costo Medio €"].to_numpy(dtype=float)
    spread = np.nan_to_num(s["Spread (P90-P10) €"].to_numpy(dtype=float), nan=0.0)
    s["Score (min meglio)"] = mean + float(risk_aversion) * spread

    if last_possible_start is None: return None, s, "Finestra temporale insufficiente"
    candidates = s[pd.to_datetime(s["Giorno Inizio (D0)"]) <= pd.Timestamp(last_possible_start.date())].copy()
    if candidates.empty: return None, s, "Data minima D0 oltre ultimo giorno utile"
    if np.nanmax(candidates["Probabilità Successo (%)"].to_numpy(dtype=float)) <= 0.0: return None, s, "Probabilità successo 0% per tutti i D0"

    best_idx = int(np.nanargmin(candidates["Score (min meglio)"].to_numpy(dtype=float)))
    return pd.Timestamp(candidates.iloc[best_idx]["Giorno Inizio (D0)"]), s, ""
...
with st.sidebar:
    ...
    model = st.selectbox("Modello", options=["gfs_seamless", "icon_seamless", "ecmwf_ifs025_ensemble"], index=2)
...
try:
    t0, t1 = to_time(shift_start), to_time(shift_end)
    if t0 >= t1:
        st.error("Orari turno non validi.")
        st.stop()
except Exception:
...
for _, r in steps_df.iterrows():
    ...
    dur, wt = safe_float(r.get("Durata [h]")), safe_float(r.get("Wind Threshold [m/s]"))
    minw = safe_float(r.get("Finestra minima consecutiva [h]"), default=0.0)
    gt_raw = r.get("Gust Threshold [m/s] (opzionale)")
    gt = None if pd.isna(gt_raw) else safe_float(gt_raw)
    req = safe_bool(r.get("Richiede Gru"), default=True)

    if not name and dur is None and wt is None: continue
    if dur is None or wt is None or dur <= 0 or wt <= 0: continue
    steps.append(Step(name=name or "Step", duration_h=float(dur), wind_thr=float(wt), gust_thr=gt, min_seq_h=float(minw), requires_crane=req))
...
if actual_forecast_days < int(forecast_days) and not use_mock:
    st.warning(f"⚠️ Il modello selezionato ha restituito {actual_forecast_days} giorni di forecast. Verrà usato SOLO l'orizzonte reale disponibile.")

wind_cols_all, gust_cols_all = detect_members(df)
if not wind_cols_all:
    st.error("Nessun membro ensemble trovato.")
    st.stop()
...
# -----------------------------
# GRAFICI METEO (Sempre visibili e ordinati)
# -----------------------------
st.subheader("Contesto meteo & produzione (sempre visibile)")
st.plotly_chart(plot_wind_speed_ensemble(df_view, wind_cols_use, horizon_start, forecast_end), use_container_width=True)
st.plotly_chart(plot_expected_production(df_view, wind_cols_use, horizon_start, forecast_end), use_container_width=True)

with st.expander("Visualizza Power Curve di Riferimento"):
    st.plotly_chart(plot_power_curve(), use_container_width=True)

# -----------------------------
# MOTORE DI OTTIMIZZAZIONE E SIMULAZIONE
# -----------------------------
...
m2.metric("Membri ensemble", len(wind_cols_use))
m3.metric("Orizzonte (ore)", horizon_hours)
m4.metric("Simulazioni", len(all_days) * len(wind_cols_use))
st.info(f"⏱️ Tempo stimato: {format_time_estimate(est_sec)}")

cur_hash = hashlib.md5(json.dumps({
    "m": len(wind_cols_use), "d": [str(x.date()) for x in all_days],
...
if "sims_cost" not in st.session_state: st.session_state["sims_cost"] = None
if "sims_hash_cost" not in st.session_state: st.session_state["sims_hash_cost"] = None
...
if st.button("▶ Esegui simulazione", type="primary", use_container_width=True):
    ...
    for i, d0 in enumerate(all_days):
        sims[pd.Timestamp(d0)] = simulate_single_start_day_cost(df, pd.Timestamp(d0), wind_cols_use, gust_cols_use, steps, params)
...
if st.session_state["sims_cost"] is None: st.stop()
...
if best_d0 is None:
    ...
    c4.metric("Ultimo D0", str(last_possible_start.date()) if last_possible_start else "Nessuno")
else:
    rb = scored[scored["Giorno Inizio (D0)"] == best_d0.date()].iloc[0]
    c1.metric("Miglior D0", best_d0.strftime("%d/%m/%Y"))
    c2.metric("Costo medio atteso", f"{rb['Costo Medio €']:,.2f} €")
    c3.metric("Probabilità successo", f"{rb['Probabilità Successo (%)']:.1f} %")
    c4.metric("Ultimo D0", str(last_possible_start.date()) if last_possible_start else "Nessuno")

st.subheader("Tabella completa")
st.dataframe(
    scored[["Giorno Inizio (D0)", "Probabilità Successo (%)", "Costo P10 €", "Costo Medio €", "Costo P90 €", "Spread (P90-P10) €", "Strutturalmente impossibile"]]
    .style.format({
        "Probabilità Successo (%)": "{:.1f}", "Costo P10 €": "{:,.2f}", "Costo Medio €": "{:,.2f}",
        "Costo P90 €": "{:,.2f}", "Spread (P90-P10) €": "{:,.2f}"
    }, thousands=".", decimal=","),
    use_container_width=True, hide_index=True
)

st.subheader("Costo totale vs D0")
plot_df = scored[~scored["Strutturalmente impossibile"]].copy()
if plot_df.empty: st.warning("Nessun D0 è plottabile.")
else: st.plotly_chart(plot_cost_candles(plot_df), use_container_width=True)

st.subheader("Dettaglio D0 selezionato")
selected = st.selectbox("Seleziona D0", [d.date() for d in all_days])
...
st.caption(f"D0 selezionato: {'STRUTTURALMENTE IMPOSSIBILE' if sel_infeasible else 'Completamento possibile'} ({sel_avail} h disponibili)")
...
if sim_sel and sim_sel.get("status") == "ok":
    ...
    dcols = st.columns(4)
    dcols[0].metric("Successo", f"{success_prob:.1f} %")
    dcols[1].metric("Costo medio", f"{np.nanmean(cost_arr):,.2f} €")
    dcols[2].metric("Costo P10", f"{np.percentile(cost_arr, 10):,.2f} €")
    dcols[3].metric("Costo P90", f"{np.percentile(cost_arr, 90):,.2f} €")
    
    frac = aggregate_gantt(sim_sel["member_logs"])
    if not frac.empty: st.plotly_chart(plot_gantt_fraction(frac), use_container_width=True)
